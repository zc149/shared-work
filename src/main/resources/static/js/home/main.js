        ...
        todoList.forEach(function (todo) {
            tbody.append('<tr>' +
                ...
                '<div class="icon-container" style="margin-right: 8px; cursor: pointer;">' +
                    '<svg class="icon unchecked" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">' +
                        '<path d="M14 1a1 1 0 0 1 1 1v12a1 1 0 0 1-1 1H2a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1zM2 0a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2z"/>' +
                        '<path d="M10.97 4.97a.75.75 0 0 1 1.071 1.05l-3.992 4.99a.75.75 0 0 1-1.08.02L4.324 8.384a.75.75 0 1 1 1.06-1.06l2.094 2.093 3.473-4.425z"/>' +
                    '</svg>' +
                    '<svg class="icon checked" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16" style="display: none;">' +
                        '<path d="M2 0a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2zm10.03 4.97a.75.75 0 0 1 .011 1.05l-3.992 4.99a.75.75 0 0 1-1.08.02L4.324 8.384a.75.75 0 1 1 1.06-1.06l2.094 2.093 3.473-4.425a.75.75 0 0 1 1.08-.022z"/>' +
                    '</svg>' +
                '</div>' +
                '<span data-id="' + todo.todoId + '" style="flex-grow: 1;">' + todo.content + '</span>' +
...
$(document).on('click', '.icon-container', function() {
    ...
    let todoData = { status: false }; // 기본값: 체크 해제 상태

    if (uncheckedIcon.is(':visible')) {
        span.css('text-decoration', 'line-through'); // 체크된 경우
        uncheckedIcon.hide();
        checkedIcon.show();
        todoData.status = true; // 체크된 상태로 변경
    } else {
        span.css('text-decoration', 'none'); // 체크 해제된 경우
        uncheckedIcon.show();
        checkedIcon.hide();
        todoData.status = false; // 체크 해제 상태로 변경
    }

    // 서버에 체크 상태 전송
    todoData.todoId = todoId; // todoId 추가
